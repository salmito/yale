...
function t.GetSizeSDL(width, aspect)
  local w = width
...
function t.InitSDL(ww, wh, fs)
  print(fs)
  ww, wh = t.GetSizeSDL(ww, wh / ww)
  local sdlbits = bit.bor(sdl.SDL_RESIZABLE, sdl.SDL_DOUBLEBUF )
  if fs==true then sdlbits = bit.bor(sdl.SDL_FULLSCREEN, sdl.SDL_DOUBLEBUF ) end
  ...
  windowStruct.update = function()
    ...
    windowStruct.frameMs = curr_time - prev_time + 0.00001
